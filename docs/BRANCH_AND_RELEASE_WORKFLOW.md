...
| `production` | 维护者实际生产环境 | 不得同步公共仓库；可以同步受控的 GitHub 私有仓库 | `main` 的全部通用能力、生产私有功能、关停/访问策略和生产部署覆盖层 |
| `backup/*` | 合并或部署前的临时恢复点 | 否 | 只用于本地回退，不作为长期开发分支 |

不要把 `production` 推送到公共远端 `origin`，也不要把生产域名、凭据、数据库路径、订阅数据或私有功能复制到 `main`。如需远端备份生产分支，应使用名称明确的 `private` 远端，并在推送前确认目标仓库可见性为 Private。
