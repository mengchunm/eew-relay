...
- 将同一真实地震的多个报次及不同台站消息归纳展示，查看震中、震级、深度、烈度、坐标、各报投递变化及脱敏逐用户明细；逐用户明细支持全量排序后分页、状态/关键字筛选，并使用无需横向滚动的自适应列布局；
...
修订始终与“上一次已推送版本”比较，小幅变化会累计。默认仅在震级累计变化 `0.3`、震中移动 `10 km`、深度变化 `10 km`、发震时间变化 `3 秒`、最大烈度跨档或取消状态改变时再次推送，并在标题和正文标记为修订。不同机构的烈度表示会先换算到可比档位，避免仅因量表写法不同而重复推送。若同一时段存在两个同样可能匹配的近邻地震，系统会保守地保持为不同事件，避免错误合并造成漏报。
...
- `EVENT-rREPORT-TYPE.jsonl`：逐条订阅投递明细，包含 `pushed`、`filtered`、`skipped`、`failed` 状态，过滤/失败原因、Bark 服务器、通知级别、预估烈度、距离、ETA 和发送耗时。
- `EVENT-rREPORT-TYPE.summary.json`：事件汇总，包含总订阅数、入队数、过滤数、成功数、失败数、官方/自建 Bark 分布、耗时 P50/P90/P99 和重试次数。
